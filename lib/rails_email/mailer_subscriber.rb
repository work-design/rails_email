module RailsEmail
  class MailerSubscriber < ActiveSupport::LogSubscriber
    ...
    def record(event)
      ...
      log_mailer = Email::Log.new(message_object_id: payload[:message_object_id], mailer: payload[:mailer])
    ...
    def deliver(event)
      ...
      log_mailer = Email::Log.find_or_initialize_by(message_object_id: payload[:message_object_id], mailer: payload[:mailer])
